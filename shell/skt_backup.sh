rm -r /gold_work/db_backup_bak/*
mv /gold_work/db_backup/* /gold_work/db_backup_bak/

sh /gold_home/cron/backup/goldilocks_backup.sh -m h -p /gold_work/db_backup/ id pw

#sh /gold_home/cron/backup/rm_arch.sh
...
[gold@skt-urtpimdb1s db_backup_bak]$ cat /gold_home/cron/backup/goldilocks_backup.sh
##################################################################
...
UNION ALL SELECT '@/gold_home/goldilocks_data/wal/commit.log' as CHK from dual;
quit
EOF
}
...
do
  ...
  then
    ...
    dd if=${from_file} of=${to_file}${from_file} bs=32M iflag=direct oflag=direct
    if [[ $? -eq 0 ]]
    then
