                 ...
                 int       aDataSize)
{
    ...
    sDataPtr = sPtr + sizeof(Header) + (sizeof(int) * sHdr->mDataMaxCount ) + (sPos * sHdr->mDataSize); 
    memcpy(sDataPtr, & aDataSize , sizeof(int) );
    memcpy(sDataPtr + sizeof(int), aData , aDataSize );
    sHdr->mQueue[sIdx] = sPos;     
             ...
             int  * aSize )
{
    ...
    sPos = sHdr->mQueue[sIdx];
    sDataPtr = sPtr + sizeof(Header) + (sizeof(int) * sHdr->mDataMaxCount ) + (sPos * sHdr->mDataSize);
    memcpy(aSize , sDataPtr , sizeof(int) );
    memcpy(aData , sDataPtr + sizeof(int), *(int *) aSize );
...
main()
{
    ...
    for ( ; ; )
    {
	if ( consume( sPtr, sUserData, &sDataSz ) == 0 )
        {
            printf( "%*.*s\n", sDataSz, sDataSz, sUserData );
        }    
    }
